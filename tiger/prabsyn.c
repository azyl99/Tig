...
void pr_exp(FILE *out, A_exp v, int d) {
 ...
 case A_stringExp:
   ...
   break;
 case A_callExp:
   fprintf(out, "[callExp [%s]", S_name(v->u.call.func));fprintf(out, "\n");
 ...
 case A_recordExp:
   ...
   break;
 case A_seqExp:
   fprintf(out, "[seqExp"); fprintf(out, "\n");
   pr_expList(out, v->u.seq, d+1); fprintf(out, "]");
   break;
 case A_assignExp:
   fprintf(out, "[assignExp"); fprintf(out, "\n");
...
static void pr_dec(FILE *out, A_dec v, int d) {
 ...
 case A_varDec:
   ...
   break;
 case A_typeDec:
   fprintf(out, "[typeDec"); fprintf(out, "\n");
...
static void pr_fieldList(FILE *out, A_fieldList v, int d) {
 ...
 if (v) {
   ...
   pr_field(out, v->head, 0); fprintf(out, "\n");
...
static void pr_expList(FILE *out, A_expList v, int d) {
 indent(out, d);
 if (v) {
   fprintf(out, "[expList"); fprintf(out, "\n"); 
...
static void pr_fundecList(FILE *out, A_fundecList v, int d) {
 indent(out, d);
 if (v) {
   fprintf(out, "[fundecList"); fprintf(out, "\n"); 
...
static void pr_decList(FILE *out, A_decList v, int d) {
 indent(out, d);
 if (v) {
   fprintf(out, "[decList"); fprintf(out, "\n");
...
static void pr_efieldList(FILE *out, A_efieldList v, int d) {
 ...
 if (v) {
   ...
   pr_efield(out, v->head, 0); fprintf(out, "\n");
