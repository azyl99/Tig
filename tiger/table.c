...
struct binder_ {void *key; void *value; binder next; void *prevtop;};//next is the binder of same hash value.
struct TAB_table_ {
  binder table[TABSIZE];
  void *top;// last used binder
};
...
void *TAB_pop(TAB_table t) {
  ...
  b = t->table[index];
  assert(b);
  t->table[index] = b->next;// pop last binder: index=1(TABSIZE=8): 17->9->25->1  =>  9->25->1
  t->top=b->prevtop;
  return b->key;
}
...
void TAB_dump(TAB_table t, void (*show)(void *key, void *value)) {
  ...
  t->top=b->prevtop;
  show(b->key,b->value);//函数的名字即为地址，不需要: (*f)()，直接 f() 就好
  TAB_dump(t,show);
