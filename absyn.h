...
typedef enum {A_plusOp, A_minusOp, A_timesOp, A_divideOp, A_andOp, A_orOp, 
...
A_dec A_TypeDec(A_pos pos, A_nametyList type);				// tydec -> type type-id = ty {type type-id = ty}
A_ty A_NameTy(A_pos pos, S_symbol name);					// ty -> name
A_ty A_RecordTy(A_pos pos, A_fieldList record);				// ty -> {tyfields}
A_ty A_ArrayTy(A_pos pos, S_symbol array);					// ty -> array of type-id，相当于 "typedef type-id[] ty;"
A_field A_Field(A_pos pos, S_symbol name, S_symbol typ);	// tyfield -> name : typeid
A_fieldList A_FieldList(A_field head, A_fieldList tail);	// tyfields-> tyfield, tyfields
A_expList A_ExpList(A_exp head, A_expList tail);
A_fundec A_Fundec(A_pos pos, S_symbol name, A_fieldList params, S_symbol result,
		  A_exp body);
A_fundecList A_FundecList(A_fundec head, A_fundecList tail);
A_decList A_DecList(A_dec head, A_decList tail);			// desc -> {dec}
A_namety A_Namety(S_symbol name, A_ty ty);					// namety -> name: ty
A_nametyList A_NametyList(A_namety head, A_nametyList tail);// nametyList -> namety, nametyList，函数参数列表
